...
load(here::here("data/toad_as_character.RData")) #should work regardless

#data("toad_as_character") #this would load from installed packaged
...
It should be noted that the specific hypothesis being tested is not whether toads are predominantely right-handed or left-handed; rather, whether there is an overall tedency in the population to favor one hand, which could be right handed or left handed.  
...
In the subsequent steps of this tutorial we'll reformat these numbers into a particular structure that will represent what is known as a **sampling distribution**.  This will allow us to calcualte the **p-value**, which will allow us to assess the hypothesis of whether toads preferentially use one paw or not.
...
question("What do you think the prop.X column stands for ",
    answer("Proportion X, the proportion of toads that were right handed",
           correct = TRUE),
...
Let's step away from toads for a second and think about something you probably have more experience with: coin flipping.  
...
If toad are no more likely to be right handed than left handed, you'd probably expect that if you collected 18 toads, 9 would be right handed, and 9 would be left handed.  If this is how the experiment played out 50% would be righties. We can fill in 0.5 in the **prop.X** column.

While we might *expect* to get 9 toads that are right handed if the null was true, how *likely* is it to actually get 9 toads?  That is what is the **probability** of 9 toads (The **prob.X** w/ a **"b"**).
...
## What is the probability of getting -about- 9 toads?

* Its pretty unlikely to get exactly 9 right handed toads.  What about getting around 9 toads, eg, between 7 and 11?
* We can test this but calculating the probabilities of all of these different options and adding them up
...
* We can fill in the proportion.X column as 1 (18/18 = 1).  
* Now, What is the probability of this occuring (prob.X)?
...
## Writing out really really small probabilities
...
What about just 17 right handers and only 1 leftie? This is just a touch more likley: 6.87E-05 or 6.87 * 10^-5 or 0.0000687.  Its so small, though, it basically is still rounded to zero.
...
(or, for us, sampling 18 toads and getting all lefties when
...
* Use BINOM.TEST() to calculate this in Excel (the easiest way to do it woudl be ot just copy and paste one of your previously used formulas)
...
For the sake of completeness we'll fill in a few more situations.  First, getting just 1 right hander and  17 lefties is the converse of getting 18 right handers and just 1 leftie.  Again, very small, but not exaclty 0.000, it just gets rounded to that.
...
Moving forward, it will be important to consider both what we happened to observed and the converse or mirror image event.

When thinking about whether toads might be right or left handed, we are biased by the fact that humans are right handed.  But before we did the experiment, we might not have an expectation of whether, if a bias exists, its towards right-handedness or left-handedness.  We therefore should consider whether a bias of any kind exists.
...
* If I am concerned that a coin is not fair I am probably not concerned that it produces an exact number of heads for a given number of flips 
* That wouldn't be a rigged coin, but straight up magic!
* what I'd be concerned with is that it produces an unusual number of heads
...
When we add up probabilites we get about 0.015.  This is our  "tail probability."  We'll explain what its that tail of later.
...
As is convection x = 14, the number of events or successes (right-handed toads) and n = 18, the number of trials.  "p = 0.5" represents the null hypothesis.
...
R provides some additinal information, beyond what we've talked about.  One thing is the "95 percent confidence interval".  This is listed as 0.5236 and 0.9359.  This confidence interval brackeds the observed probability of success, 0.778.  It represents other plausible values for the proportion of right-handed toads in the population, given that we have a sample size of only 18.  Note that it doesn't contain the null value of 0.05.

## Looking ahead: logistic regression

### This tidbit is optional

A modern extension of binomial tests is **logistic regression.**  Logistics regression is a type of **generalized linear model**, or **glm** for short.  We can implement a the logistic regression version of our binomail test on our data using the **glm()** function in R.  THere are several types of **glm**s, so we need to tell R "family = 'binomail'" so it knows that we have binomial data.

The output from a glm is much more compliated.  Can you spot the p-value inthe mix?  Its actually less than 0.031, the value form the binomail test, but close.
